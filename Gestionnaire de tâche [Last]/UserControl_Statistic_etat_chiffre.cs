...
{
    public partial class UserControl_Statistic_etat_chiffre : UserControl
    {
        public Etat Etat { get; }
        public Projet Projet { get; }
        public Statistique_nombre Statistique_Nombre { get; }
        public int nbDeFoisUtilisées = 0;

        public UserControl_Statistic_etat_chiffre(Etat etat, Projet projet, Statistique_nombre statistique_Nombre)
        {
            InitializeComponent();
        ...
        private void initializeForAll()
        {
            ...
            foreach (var sousProjet in Projet.sous_Projets)
            {
                ...
                {
                    if (Etat.Equals(Main.Utilities.resource.etats[tache.State_de_la_tâche]))
                        nbFoisUtilisé++;
                }
            }
            ...
            if (nbTache > 1)
                label_chiifre1.Text = "Utilisé " + nbFoisUtilisé.ToString() + " fois sur " + nbTache + " tâches (" + value + "%)" + " dans ce projet";
            else
                label_chiifre1.Text = "Utilisé " + nbFoisUtilisé.ToString() + " fois sur " + nbTache + " tâche (" + value + "%)" + " dans ce projet";
        ...
        private void UserControl_Statistic_etat_chiffre_Load(object sender, EventArgs e)
        {
        }

        private void label_chiifre1_Click(object sender, EventArgs e)
        {
        }
        ...
        private void comboBox_sousProj_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!Statistique_Nombre.onlyMoreThan0)
            {
                ...
                Statistique_Nombre.SetAllComboBoxFor(comboBox_sousProj.SelectedIndex);
            }
            else
            {
                comboBox_sousProj.SelectedIndex = 0;
        ...
        private void label_chiffre2_Enter(object sender, EventArgs e)
        {
            this.ActiveControl = labelstate;
        }
    }
